...
class ImagePanel(QWidget):
    ...
    def show(self, original, processed):
        if original is None or processed is None:
            self.clear()
            return
        original_ref = id(original)
        processed_ref = id(processed)
        if self._original_ref != original_ref:
            self.draw(self.s1, self.v1, original)
            self._original_ref = original_ref
        if self._processed_ref != processed_ref:
            self.draw(self.s2, self.v2, processed)
            self._processed_ref = processed_ref
