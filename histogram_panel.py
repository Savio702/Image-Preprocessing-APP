...
class HistogramPanel(QWidget):
    ...
    def redraw(self):
        ...
        if self.original is None or self.processed is None:
            self.canvas.clear()
            return
        original_series = self.build_histogram_series(self.original, self.original_mode.currentText())
        processed_series = self.build_histogram_series(self.processed, self.processed_mode.currentText())
